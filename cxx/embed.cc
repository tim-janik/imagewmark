...
static std::pair<int, std::vector<float>>
make_wm_pattern()
{
...
static VImage
image_from_floats (const std::vector<float> &floats, int width, int height, int bands = 1)
{
  const size_t size = floats.size() * sizeof (float);
  void *memory = malloc (size);
  if (!memory)
    throw std::bad_alloc();
  std::memcpy (memory, floats.data(), size);
  return VImage::new_from_memory_steal (memory, size, width, height, bands, VIPS_FORMAT_FLOAT)
    .copy (VImage::option()->set ("interpretation", bands == 1 ? VIPS_INTERPRETATION_B_W : VIPS_INTERPRETATION_sRGB));
}

static VImage
box_kernel (int width, int height)
{
  std::vector<double> coeffs (width * height, 1.0 / (width * height));
...
local_mean (const VImage &img, int win_width, int win_height)
{
  ...
  return padded.convf (box_kernel (win_width, win_height))
    .crop (pad_x, pad_y, img.width(), img.height());
}
...
{
  VImage mean1 = local_mean (img, Config::var_win_w, Config::var_win_h);
  VImage mean2 = local_mean (img * img, Config::var_win_w, Config::var_win_h);
...
{
  const VImage var_clamped = (img_variance < 1e-5).ifthenelse (1e-5, img_variance);  // enforce ≥ 1e-5
  const VImage log2_var = var_clamped.pow (0.5).math (VIPS_OPERATION_MATH_LOG) / std::log (2.0);
  return (log2_var < strength).ifthenelse (strength, log2_var);         // max(strength, log₂(√var))
}
...
{
  const float s = std::max (strength, std::log2 (std::sqrt (1e-5f)));
  VImage abs_scaled = (W * s).abs();
  for (int r = 0; r < 32; r++) {
...
{
  return (img + 0.5).cast (VIPS_FORMAT_UCHAR);
}
...
command_add (const AddOptions &opt)
{
  ...
  std::vector<float> wmunit (Lwsmall * Lwsmall);
  ...
  const std::vector<float> Ksmall = make_wm_mask (Lwsmall);
  // Mask wmunit with Ksmall
  std::vector<float> wmasked (wmunit.size());
  for (size_t i = 0; i < wmasked.size(); ++i)
    wmasked[i] = wmunit[i] * Ksmall[i];
  ...
  const int tiled_width = Lwsmall * hreps;
  const int tiled_height = Lwsmall * vreps;
  std::vector<float> tiled_img (tiled_width * tiled_height);
  for (int v = 0; v < vreps; ++v)
    for (int h = 0; h < hreps; ++h) {
      for (int y = 0; y < Lwsmall; ++y)
        for (int x = 0; x < Lwsmall; ++x) {
          const int sy = v % 2 ? Lwsmall - 1 - y : y;
          const int sx = h % 2 ? Lwsmall - 1 - x : x;
          tiled_img[(v * Lwsmall + y) * tiled_width + h * Lwsmall + x] = wmasked[sy * Lwsmall + sx];
        }
    }
  // Resize to the host size, see: common.py:zoom_image()
  VImage W = image_from_floats (tiled_img, tiled_width, tiled_height)
    .resize (ZOOM, VImage::option()->set ("vscale", ZOOM)->set ("kernel", VIPS_KERNEL_CUBIC));
  ...
  // Write the result
  watermarked.write_to_file (opt.output_img.c_str(),
...
imagewmark_add (const AddOptions &options)
{
  ...
  if (!vips_initialized) {
    if (VIPS_INIT (argv0 ? argv0 : "imagewmark"))
  ...
  try {
    ...
    die (1, "libvips failure: %s", error.what());
  }
  return 0;
}
