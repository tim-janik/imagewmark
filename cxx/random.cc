  ...
  if (!init_ok)
    {
      ...
      if (!gcry_check_version (GCRYPT_VERSION))
        die (5, "libgcrypt version mismatch\n");
...
Random::load_global_key (const string& key_file)
{
  ...
  if (!f)
    die (5, "failed to open key file: '%s'\n", key_file.c_str());
      ...
      if (regex_match (s, blank_re))
      ...
      else if (regex_match (s, match, key_re))
        {
          ...
          if (key.size() != aes_key.size())
            die (5, "wrong key length in key file '%s', line %d\n => required key length is %zd bits\n", key_file.c_str(), line, aes_key.size() * 8);
          aes_key = key;
          keys++;
        }
      else
        {
          die (5, "parse error in key file '%s', line %d\n", key_file.c_str(), line);
          exit (1);
        }
      line++;
    }
  ...
  if (keys > 1)
    die (5, "key file '%s' contains more than one key\n", key_file.c_str());
  if (keys == 0)
    die (5, "key file '%s' contains no key\n", key_file.c_str());
}
